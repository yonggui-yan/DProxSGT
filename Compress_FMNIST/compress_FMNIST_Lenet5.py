...
from lenet_FMNIST import LeNet5
...
parser.add_argument('--maxmax', type=float, default=1.0)
...
def main():
    ...
    dir = 'FMNIST_LENET5_Size'+str(SIZE) +'_epochs_'+str(epochs) +'_batchsize'+str(batch_size) + '_lr0'+str(lr0) +'_data_divide'+str(args.data_divide)+'_method'+str(args.method) 
...
#        sys.path.append('..')
    ...
    if args.method=='DProxCGT2':
        ...
        timer = Timer(total_time = total_time)
        train_DProxCGT2(model,epochs, compressor_gy, gamma_y, train_loader,test_loader,opt,train_loader_all, timer=timer,loggers=(TableLogger(), TSV), Model_Path=dir,epoch0=args.epoch0, beta=args.beta)
    ...
    if args.method=='DProxCGT3':
        ...
        train_DProxCGT3(model,epochs, compressor_gy, gamma_y, train_loader,test_loader,opt,train_loader_all, timer=timer,loggers=(TSV,), Model_Path=dir,epoch0=args.epoch0, beta=args.beta, maxmax=args.maxmax)
...
# run by "mpiexec -np 3 python demo_mult_node.py
